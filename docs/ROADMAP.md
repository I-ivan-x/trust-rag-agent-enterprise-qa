...
| Q3-P7 | Codex + Owner + Claude | 对比 run（rule vs llm，k=3）+ 报告（三段式 + taxonomy 扩展） | 🟡 harness ✅ `614d548`；真实 run ✅ `q3-p7-governance-ablation`（安全层 blocked=1.00/F11=F13=0；triad=False→headline 不合格；rule≈llm）；**报告待 Claude**（EVAL Q3 节 + FAILURE F10–F13） |
| Q3-P8 | Codex | Web 动作控制台（读→判→动→治 时间线 + 待审队列 + 审计链 + 越权拦截日志） | ⏳ `SPEC_Q3_P8.md`（govern_routes 六端点 + 4 视图 + canned 演示脚本） |
