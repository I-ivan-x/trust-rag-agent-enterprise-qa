...
> 这个 split 的定位就是压力测试：20 对相邻主题/相似标题的公开文档，专门构造来打检索。
> 旧测试得到 doc_hit@5 只有 0.05、error rate 1.0；这是真实记录的严重评测失败，
> 但后续裁定证明它不能作为检索能力结论。
>
...
## Q16：延迟、吞吐和并发怎么样？能报 p95 或 QPS 吗？

**推荐回答：**

> 不能。我没有做生产负载基准，因此不报告 p95、QPS、并发上限或容量规划数字。
> 当前评测关注正确性、权限边界、调用量和可复现发布；单机开发运行也不能替代
> 稳态压测。主要延迟来源会是 provider、embedding、reranker、索引访问和审批等待，
> 其中人工审批更应按工作流 SLO 单独统计，而不是混进模型延迟。
>
> 真正上线前，我会先固定硬件、模型与语料规模，分别测冷/热缓存下的 retrieval、
> generation 和 end-to-end histogram，再用受控并发逐级压测，记录 p50/p95/p99、
> saturation、错误率、队列深度、token 成本和被限流比例；同时验证超时、重试、幂等、
> backpressure 与审批积压。没有这组数据前，最诚实的结论就是“性能尚未评估”。
...
> 等于替攻击者把载荷送进上下文。10 条投毒/干净成对对照里，ACL、state 和
> permission 的 host gate 保持原行为，但语义回答层并没有过关：4/10 从安全模式
> 退化为 `answer`，其中 1/10 实际采纳恶意主张；6/10 直接拒答。chunker 还保留了
> HTML comment 和 155 个 zero-width 字符，说明 ingest 未完成规范化。
>
> 所以我不会说系统“免疫 prompt injection”。当前证据只说明权限与副作用边界比
> 语义回答层更稳；metadata-only 冲突决策、HTML/zero-width 清洗和注入分类仍未实现。
> 这也是项目不部署、不声称 production-ready 的直接证据。
