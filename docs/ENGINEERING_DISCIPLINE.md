...
| 全量回归 | **552 passed, 1 skipped** | 2026-07-13 独立实跑；另有 23 条 warning |
...
| 工程质量 | 552 passed / 1 skipped、Ruff 全绿、Docker smoke、run manifest、trace、失败分类与回归检查脚本 |
...
- 全量测试为 552 passed / 1 skipped，但仍有 23 条 warning，需要继续清理依赖弃用、mock 提示和缓存权限问题。
...
- v3 唯一 DeepSeek real-dev 证明 observation、去重、安全和效率均成立，但 semantic uplift 仅
  `0.083333`，within/cross 仅 `0.166667 / 0.333333`。真实轨迹显示模型能复述命中的 policy clause，
  action 却与 clause 冲突；项目因此继续拒绝 freeze，并将下一步限定为 typed policy disposition 与
  deterministic action compilation，而不是放宽 Gate 或重复刷分。
...
> 完整测试 552 passed，并将未达门槛的 judge/Agent 能力主动证伪、降级或锁定。
