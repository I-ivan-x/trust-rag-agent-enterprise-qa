...
policy-crowding 信号（deprecated/restricted 邻居 ≥2 且 clean 不足） → 使 b 合法
两者同时出现                                                       → 合法集 = {a, b, e}
```
...
**测试床硬要求（§8）**：定向 case 必须含 ≥6 条信号共现 case（a、b 同时合法），
否则消融无决策点、退化为"恢复 vs 拒答"。这是 agent 测试床最重要的属性，W7 出题
时优先保证。
...
定向 case **必须含 ≥6 条 a/b 信号共现 case**（§3.1）——否则消融无决策点、退化为
"恢复 vs 拒答"，决议 A 跑不出有信息量的结论。这是测试床的首要属性。
