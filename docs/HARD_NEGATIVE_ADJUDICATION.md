...
## 4b. 裁定更新（2026-06-15，含外部分析采纳项）

冻结名：`hard_negative_rewritten_v1`（18 条；019/020 弃用）。

**query 修正（4 条）：**

- **002**（gold=Cookie 侧）：草稿误问 header → 改为
  "How do I read a value from a request **cookie** inside my endpoint function?"
- **005**（gold=Request Files，对侧 Request Forms and Files）：加排他限定 →
  "How do I declare a parameter for a single uploaded file in an endpoint **without
  any form fields**?"
- **007**（gold=JSON Compatible Encoder，对侧 Body-Updates）：去 database/update 味 →
  "How do I convert a Pydantic model containing a `datetime` into a JSON-compatible
  `dict` using `jsonable_encoder`?"
- **016**（gold=Query Parameters and String Validations）：`regex`→`pattern`（贴新版）→
  "How do I add a max length or **pattern** validation to a query parameter?"

其余 14 条按 §4 草稿，待 Owner 最终签字。

**难度分档（pre-run 预测，复跑后据 doc_hit@5 复核，不得当测量值报告）：**

- A（gold 明确、对侧不应满足）：001,003,004,006,008,009,010,012,013,014,015,017,018
- B（真人会这么问、对侧可能部分满足，重点观察）：005,007,011,016
- 002 修正后入 A。

**11 / 015 为"重写后仍真难"的优质难例**：若复跑后仅此类失败 → F3 真实信号。

## 4c. Owner 最终签字（2026-06-15，权威版，覆盖 §4/§4b 草稿）

Codex 用此 18 条作为 `hard_negative_eval.jsonl` 的 query（gold 不变）。
诊断字段（gold_distinguishers / anti_should_not_satisfy_because /
expected_answer_focus）从 Owner 签字消息逐条转入 a2 annotations 文件，非评分。

```text
001 How can I show example JSON payloads for a request body in the generated API docs?
002 How do I declare a cookie parameter so my endpoint reads a value from the request cookies?
003 Can I make FastAPI use my path function's return type annotation as the response schema?
004 My endpoint receives an HTML form POST instead of JSON. How do I declare those form fields?
005 How do I declare an endpoint parameter for accepting a single uploaded file, without any additional form fields?
006 What options can I pass to the route decorator to set tags, summary, description, or response status code?
007 How do I use jsonable_encoder to convert a Pydantic model, datetime, or other Python object into JSON-compatible data?
008 Can I use a class instead of a function as a dependency, and what does FastAPI instantiate or return from it?
009 Can a dependency itself depend on another dependency, and how does FastAPI resolve that dependency chain?
010 How do I run cleanup code after a request finishes, like closing a database session opened by a dependency?
011 Which authentication standards and mechanisms do FastAPI's security utilities cover, such as OAuth2, OpenID Connect, and API keys?
012 What is the simplest way to wire up a username/password login that returns a bearer token?
013 How do I issue and verify JWT access tokens with hashed passwords for a login flow?
014 What does the minimal FastAPI hello-world app file look like, and how do I run it locally?
015 How do I capture a variable segment of the URL path as a typed function argument?
016 How do I add max length or pattern validation to a string query parameter?
017 How do I enforce numeric constraints like greater-than-or-equal on a path parameter?
018 How do I define a request body model that contains a list of other models nested inside it?
```

复核 watch-items（非阻塞，泄漏检查兜底）：
- **011**：query 列举了 OAuth2/OpenID Connect/API keys，可能与 gold 正文用词重合较高；
  若 answer-copy 或 title-overlap flag 触发 → 删掉枚举改为"such as common token-based
  and key-based schemes"。retrieval-only 不受影响（枚举是指向 gold 的有效信号）。
- **跨 case 碰撞是设计内、且被正确度量**：如 006 含 "response status code"、015/016/017
  都关于 parameters，检索是对全语料跑的，可能互相召回；但 doc_hit@5 按各自 gold 计，
  碰撞=正确记为 miss。这正是该 split 要测的"在众多相似页中找对页"，不是缺陷。
...
## 7. Codex 执行结果（2026-06-15）

- `data/gold_eval/hard_negative_eval.jsonl` 已按 §4c 收缩为 18 条；
  `query_source=manual_adversarial_rewrite`，gold doc/chunk/reference 字段保持不变。
- 非评分 annotation 已写入
  `data/gold_eval/hard_negative_rewritten_v1_annotations.jsonl`；这些字段仅用于人工审计
  与 trajectory 归因，不进检索、打分或 `EvalCase` schema。
- 双向泄漏检查已扩展并通过：`docs/EVAL_LEAKAGE_REPORT_C2_05_HARDNEG_REWRITTEN.md`
  记录 18 条、0 个 blocking flag；gold 校验确认 18 条 gold doc/chunk ID 均存在于当前
  hard-negative chunks。
- 零 token retrieval-only 复跑 `q2-c205-hardneg-rewritten-retrieval` 已落盘：
  vector_only / bm25_only / hybrid_rrf / hybrid_rrf_rerank 的 `doc_hit@5` 均为 1.0000。

结论：F8 被确认是 Week 6 hard-negative 失败主因；F3 未在旧 split 中被有效测到。
重写后的 n=18 是新的公平测量，不能包装成相对 Week 6 的"提升"。
