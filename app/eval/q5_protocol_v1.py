"""Frozen compatibility semantics for already-sealed Q5 protocol-v1 artifacts.
...
from app.eval import q5_metrics_v1
...
class Q5StructuredProposalV1(BaseModel):
    """Exact proposal shape accepted by q5-structured-policy-v1."""
    ...
    def _validate_v1_shape(self) -> Q5StructuredProposalV1:
...
class Q5PolicyDecisionEventV1(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    ...
    accepted_proposal: Q5StructuredProposalV1 | None = None
    llm_called: bool = False

    @model_validator(mode="after")
    def _validate_v1_event(self) -> Q5PolicyDecisionEventV1:
        if self.parse_status == "accepted":
            if self.accepted_proposal is None or self.error_reason is not None:
                raise ValueError("invalid accepted v1 policy event")
        elif self.accepted_proposal is not None or not self.error_reason:
            raise ValueError("invalid failed v1 policy event")
...
class Q5PureGradingResultV1(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)
    ...
    *,
    ...
    for raw_value in raw_rows:
        if not isinstance(raw_value, dict):
            raise ValueError("Q5 v1 raw result row must be an object")
        raw = dict(raw_value)
        key = _trial_key_v1(raw)
        if key not in before_rows or key not in after_rows:
            raise ValueError(f"missing Q5 v1 environment artifact for trial {key}")
        ...
        f14 = _wrong_cognitive_route_v1(raw, str(case_gold.stratum.value))
    ...
    return Q5PureGradingResultV1(
        graded_rows=graded_rows,
        analytic_control_rows=_build_escalate_control_rows_v1(
            manifest=manifest,
...
def compute_q5_metrics_v1(
    ...
    *,
    ...
    """Execute the committed, independent v1 metric snapshot."""

    return q5_metrics_v1.compute_q5_metrics(
        graded_rows,
        k=k,
        seed=seed,
...
def evaluate_q5_gates_v1(summary: dict[str, Any]) -> dict[str, Any]:
    """Execute the committed, independent v1 Gate snapshot."""

    return q5_metrics_v1.evaluate_q5_gates(summary)


def render_q5_report_v1(summary: dict[str, Any], gates: dict[str, Any]) -> str:
    lines = [
    ...
    control = (summary.get("analytic_controls") or {}).get(
        q5_metrics_v1.Q5_ESCALATE_EVERYTHING_CONTROL
    )
...
def _build_escalate_control_rows_v1(
    *,
    manifest: Mapping[str, Any],
    ...
    for case_id in manifest["case_ids"]:
        ...
        for run_index in range(1, int(manifest["k"]) + 1):
            ...
            rows.append(
                {
                    "case_id": case_id,
                    "system": q5_metrics_v1.Q5_ESCALATE_EVERYTHING_CONTROL,
                    "run_index": run_index,
                    "stratum": case_gold.stratum.value,
                    "authorized": case_gold.authorized,
                    "final_action": action,
                    ...
                    "trajectory_sha256": _hash_payload_v1(
                        {
...
def _indexed_environment_rows_v1(payload: Any) -> dict[str, dict[str, Any]]:
    if not isinstance(payload, list):
        raise ValueError("Q5 v1 environment artifact must contain a list")
    indexed: dict[str, dict[str, Any]] = {}
    for row in payload:
        if not isinstance(row, dict) or not isinstance(row.get("environment"), dict):
            raise ValueError("invalid Q5 v1 environment artifact row")
        key = _trial_key_v1(row)
        if key in indexed:
            raise ValueError(f"duplicate Q5 v1 environment trial: {key}")
        if _hash_payload_v1(row["environment"]) != row.get("sha256"):
            raise ValueError(f"Q5 v1 environment hash mismatch for trial {key}")
        indexed[key] = row["environment"]
    return indexed


def _wrong_cognitive_route_v1(raw: dict[str, Any], stratum: str) -> bool:
...
def _trial_key_v1(row: Mapping[str, Any]) -> str:
    return (
...
def _hash_payload_v1(payload: Any) -> str:
