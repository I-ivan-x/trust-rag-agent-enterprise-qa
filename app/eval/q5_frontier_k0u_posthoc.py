"""Independent runtime-only challenger authored after the K0U data commit."""
...
_ACTION_CUES = {
    ...
    FrontierDisposition.no_action: re.compile(
        r"(?:governance (?:state )?(?:unchanged|untouched)|preserv(?:e|es).{0,32}"
        r"(?:state|decision)|no governance change|leave.{0,24}unchanged)",
        re.IGNORECASE,
    ),
}


def independent_runtime_challenger(
    runtime: PracticalRuntimeInput,
) -> PracticalParserResult:
    """Compose a status predicate with two ordered deontic action cues."""
    ...
    return PracticalParserResult(
        status="complete",
        reason="composed status predicate and ordered action cues",
